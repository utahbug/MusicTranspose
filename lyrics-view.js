...
export function createLyricsView(host,{onScore,onLibrary}){
 let size=1,dark=false,current=null;
 ...
 const header=make('header');header.append(make('h1',current.title),make('p',current.collection+' · '+current.number,'lyrics-source'));host.append(header);
 if(current.notes.length)host.append(make('p','Shared ending shown separately; consult the score for repeats.','lyrics-notice'));
 ...
 return {show(data,{fresh=false}={}){if(fresh){size=1;dark=false;}current=data;draw();host.hidden=false;const heading=host.querySelector('h1');heading.tabIndex=-1;heading.focus({preventScroll:true});},hide(){host.hidden=true;},reset(){size=1;dark=false;current=null;host.hidden=true;}};
}
