...
export function createLyricsView(host,{onScore,onLibrary}){
 let size=1,dark=true,current=null,fun=false,stopFun=null;
 ...
 tools.append(theme,button(['A','A+','A++'][size],'Lyrics font size: '+['Small','Medium','Large'][size],()=>{size=(size+1)%3;host.dataset.size=['small','medium','large'][size];const control=host.querySelectorAll('.lyrics-tools button')[1];control.textContent=['A','A+','A++'][size];control.title='Lyrics font size: '+['Small','Medium','Large'][size];control.setAttribute('aria-label',control.title);}),button('♫','Open score',onScore));const funButton=button('✦','Toggle lyrics fun mode',()=>{fun=!fun;cleanup();funButton.setAttribute('aria-pressed',String(fun));if(fun)stopFun=createLyricsFun(host,host.querySelector('.lyrics-paper'));});funButton.setAttribute('aria-pressed',String(fun));tools.append(funButton);top.append(button('Library','Return to Library',onLibrary),tools);host.append(top);
 ...
 paper.append(body);if(fun)stopFun=createLyricsFun(host,paper);
 }
 return {show(data,{fresh=false}={}){if(fresh){size=1;dark=true;fun=false;}current=data;draw();host.hidden=false;const heading=host.querySelector('h1');heading.tabIndex=-1;heading.focus({preventScroll:true});},hide(){cleanup();fun=false;host.hidden=true;},reset(){cleanup();fun=false;size=1;dark=true;current=null;host.hidden=true;}};
}
