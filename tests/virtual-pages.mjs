...
try{
 ...
 for(const [width,height] of [[1180,820],[820,1180],[390,844],[1440,1000]]){
  ...
  for(const id of ['hhc-1010','cs-236','nativity','shepherd','faithful','silent-night','cs-12','cs-110','hhc-1001']){
   ...
   for(let i=1;i<result.pages;i++){await page.keyboard.press('PageDown');assert.equal(await position(),`${i+1} / ${result.pages}`);assert.equal(await page.evaluate(()=>scrollY),0);const bounds=await page.locator('.mxl-page-frame.current-page').evaluate(e=>{const svg=e.querySelector('svg');return {overflow:e.scrollHeight>Math.ceil(e.clientHeight)+2,inside:!svg||svg.getBoundingClientRect().bottom<=e.getBoundingClientRect().bottom+1};});assert(!bounds.overflow);assert(bounds.inside);}
   await page.keyboard.press('PageDown');assert.equal(await position(),`${result.pages} / ${result.pages}`);await page.keyboard.press('Home');assert.equal(await position(),`1 / ${result.pages}`);
 ...
 await page.locator('.score-heading .song-playback').click();await page.waitForFunction(()=>prototype.playback.state==='playing');await page.keyboard.press('PageDown');assert.equal(await page.evaluate(()=>prototype.playback.state),'playing');await page.locator('#show-lyrics').click();await page.getByRole('button',{name:'Open score',exact:true}).click();await ready();assert.equal(await page.evaluate(()=>prototype.playback.state),'playing');
