...
for(const [width,height,font] of [[340,650,22],[760,1000,24],[760,650,24],[760,850,24]])for(let run=0;run<40;run++){
 const w=createWorm(width,height,font);assert((w.count-1)*w.spacing/width>.35&&(w.count-1)*w.spacing/width<.45);assert(w.count<=96);assert.equal(wormHit(w,{x:w.x,y:w.y}).index,0);
