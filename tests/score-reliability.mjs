...
try{
 ...
 for(const [width,height,touch] of sizes){
  ...
  await p.goto(base);await p.locator('.library-row').first().waitFor();await p.evaluate(id=>prototype.loadSong(id),width===820||width===1440?'cs-110':'nativity');await ready(p);await mode(p,'pages');
  ...
  await p.locator('#settings').click();await p.locator('#show-tap-zones').click();const r=await geom(),box=await p.locator('#score-tap-overlay').boundingBox(),cells=await p.locator('.score-tap-grid>span').evaluateAll(es=>es.map(e=>{const r=e.getBoundingClientRect();return {x:r.x,y:r.y,width:r.width,height:r.height};}));
  ...
  const layout=await p.evaluate(()=>{const rect=id=>{const e=document.getElementById(id),r=e.getBoundingClientRect();return {x:r.x,y:r.y,w:r.width,h:r.height,visible:!!e.getClientRects().length};};return {overflow:document.documentElement.scrollWidth>innerWidth,source:rect('score-source'),lyrics:rect('show-lyrics'),key:rect('key'),original:rect('original-key-reference'),header:document.querySelector('.score-heading').getBoundingClientRect().height};});
  assert(!layout.overflow);assert(layout.lyrics.w>=44);assert(layout.key.h>=44);assert(layout.source.x+layout.source.w+8<=layout.lyrics.x+.5);assert(await p.locator('.score-paper > #original-key-reference').count());assert(layout.original.y+layout.original.h<=layout.key.y);
  await p.screenshot({path:`test-results/score-reliability-${width}.png`});
  if(width===1440){const before=await pos();await p.locator('#settings').click();await p.locator('#show-tap-zones').click();await p.locator('#score-tap-overlay').waitFor({state:'hidden',timeout:10000});assert.equal(await pos(),before);}
  ...
  await p.locator('#settings').click();await p.locator('#show-tap-zones').click();assert.equal(await p.locator('#score-tap-overlay').evaluate(e=>e.style.getPropertyValue('--tap-upper')),'25%');await p.keyboard.press('Escape');
  await p.evaluate(()=>prototype.loadSong('hhc-1054'));await ready(p);await p.locator('#score-size').click();await p.locator('[data-size=large]').click();await ready(p);assert.equal(await p.locator('#score-size').getAttribute('data-size'),'large');await tap(false,false);await tap(true,true);await p.locator('#score-size').click();await p.locator('[data-size=auto]').click();await ready(p);
  ...
  await p.locator('#settings').click();await p.locator('#show-tap-zones').click();assert.equal(await p.locator('#score-tap-overlay').evaluate(e=>e.style.getPropertyValue('--tap-upper')),'50%');await p.keyboard.press('Escape');
