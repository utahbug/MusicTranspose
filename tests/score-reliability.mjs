...
try{
 ...
 for(const [width,height,touch] of sizes){
  ...
  assert(!layout.overflow);assert(layout.lyrics.w>=44);assert(layout.key.h>=44);assert(layout.source.x+layout.source.w+8<=layout.lyrics.x+.5);assert(layout.original.y>height-130);assert(layout.original.y+layout.original.h<=layout.key.y);
