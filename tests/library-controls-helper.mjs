...
export async function setOrder(p,value){if(value==='list'){if(await p.locator('#reorder-list').getAttribute('aria-pressed')!=='true')await p.locator('#reorder-list').click();await p.locator('#reorder-list').click();}else await p.locator('#order-'+value).click();}
export async function getOrder(p){for(const v of ['title','number'])if(await p.locator('#order-'+v).getAttribute('aria-pressed')==='true')return v;return 'list';}
