...
for(const [width,height] of [[820,1180],[1180,820],[390,844],[1440,1000]]){
 ...
 for(let run=0;run<16;run++){
  await p.locator(`[data-song="${ids[0]}"] .song-view-actions button`).last().click();await p.locator('#lyrics-view').waitFor({state:'visible'});await holdLyrics(p);await p.clock.runFor(6000);
