...
export function assessLayout(host,systems,width,available,zoom){
 const svgs=[...host.querySelectorAll('svg')],groups=[];let clipping=0,collisions=0,minLyric=Infinity,oversize=0;
 ...
 for(const system of systems){const svg=svgs[system.svgIndex],scale=width/svg.viewBox.baseVal.width,last=groups.at(-1);if((system.bottom-system.top)*scale>available)oversize++;if(last&&last.svgIndex===system.svgIndex&&(system.bottom-last.top)*scale<=available){last.bottom=system.bottom;last.end=system.end;last.systems+=system.systems;}else groups.push({...system});}
 const first=groups[0];return {zoom,pages:groups.length,visibleSystems:first?.systems||0,visibleMeasures:first?first.end-first.start+1:0,totalSystems:systems.reduce((n,s)=>n+s.systems,0),clipping,collisions,minLyric:Number.isFinite(minLyric)?minLyric:null,oversize,readable:!clipping&&!oversize&&minLyric>=12.5};
}
