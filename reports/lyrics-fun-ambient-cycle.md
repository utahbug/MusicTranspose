...
- Pure-model randomized tests cover 160 worms, phase ranges, 70â€“90% length, head/body hit targeting, head-only minimum, resize, bounded history and differing paths.
- Browser tests cover 390Ã—844, 820Ã—1180, 1180Ã—820 and 1440Ã—1000: progressive removal of every body segment, surviving head, immediate head kill, four-second gap and randomized respawn, tap endpoints, varied colors, reduced motion, OFF/ON restart, exit cleanup and no horizontal overflow.
- A deterministic browser case covers natural escape and its four-second respawn gap.
- A real four-minute run (no simulated clock) traversed notes â†’ worm â†’ variation â†’ notes-return â†’ notes â†’ worm â†’ variation. It retained one overlay and at most one pending animation-loop frame, with bounded nodes and unchanged lyric text. Exit removed the frame and overlay; no browser errors occurred.
...
- Screenshots inspected at phone, both tablet orientations and desktop widths show a thin notehead chain without new layout space or game chrome.
...
The existing GitHub Pages deployment completed successfully. Live overlay/helper/CSS/service-worker files matched the committed sources. All four responsive interaction tests, natural escape/respawn, existing playback/view/print/gesture regressions and offline preference checks passed against the live URL. A live-test loading race was corrected by waiting for the Lyrics overlay before advancing the simulated clock; no application change was required.
