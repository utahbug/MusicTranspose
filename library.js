...
export function initLibrary({loadSong,openLyrics,isBusy,leaveScore,cancelPendingSelection,stopPlayback,showScoreView}){
 ...
 function open(id,view='normal'){reordering=false;if(!$('library').hidden){libraryScroll=scrollY;remember();persistWorkspace();}loadSong(id,view);}
 ...
 function refreshLists(){
  ...
  heading('Sources');
  for(const [id] of sourceChoices)item(sourceLabel(id),!activeList&&source===id,()=>{if(activeList)selectList(null);source=id;libraryScroll=0;savePreferences();render();window.scrollTo({top:0,behavior:'instant'});remember();persistWorkspace();navigation?.snapshot();},id);
  divider();heading('My Lists');
  for(const g of state.groups)item(g.name,activeList===g.id,()=>selectList(g.id),'list:'+g.id);
  divider();const edit=button('Edit Lists…',null,()=>{closeSource();remember();persistWorkspace();blurSearch();navigation?.visit({view:'lists',list:null,picking:false});files.hide();lists.open();});edit.setAttribute('role','menuitem');edit.dataset.source='edit-lists';sourceMenu.append(edit);
 }
 ...
 function render(){
  ...
  $('library-sort-label').replaceChildren(node('span','Sort: ','desktop-prefix'),node('span',sort==='manual'?'Manual':sort==='number'?'Number':'Title','desktop-sort'),node('span',sort==='manual'?'Manual':sort==='number'?'Number':'A\u2013Z','phone-sort'));
