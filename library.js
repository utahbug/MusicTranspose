...
export function initLibrary({loadSong,openLyrics,isBusy,leaveScore,stopPlayback,showScoreView}){
 ...
 $('manage-lists').onclick=()=>{navigation?.visit({view:'lists',list:lists.active});files.hide();fromList=true;lists.open();};
 $('view-files').onclick=()=>{navigation?.visit({view:'files'});fromList=false;libraryScroll=scrollY;lists.hide();files.open();};
 ...
 async function restore(route){
  ...
  else{$('library').hidden=false;document.title='MusicTranspose · Library';render();$('library-search').focus({preventScroll:true});}
