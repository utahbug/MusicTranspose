...
export function initLibrary({loadSong,openLyrics,isBusy,leaveScore}){
 ...
 function render(){
  ...
  for(const s of found){
   ...
   }else{const views=node('div',null,'song-view-actions');const scoreButton=button('♫','Open score: '+s.title,()=>open(s.id));scoreButton.title='Open score';views.append(scoreButton);if(lyricIds.has(s.id)){const lyricsButton=button('','Open lyrics',()=>{if(!isBusy())openLyrics(s.id);});lyricsButton.innerHTML='<svg class="lyrics-icon" viewBox="0 0 24 24" aria-hidden="true" focusable="false"><path d="M3 5h11M3 10h9M3 15h7M3 20h7M17 17V5l4-1v10"/><ellipse cx="14.5" cy="18" rx="2.5" ry="1.8"/><ellipse cx="18.5" cy="15" rx="2.5" ry="1.8"/></svg>';lyricsButton.setAttribute('aria-label','Lyrics');lyricsButton.title='Lyrics';views.append(lyricsButton);}row.append(star,entry,views,lists);}
   fragment.append(row);
  }
