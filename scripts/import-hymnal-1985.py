...
import csv,json,hashlib,shutil,zipfile,xml.etree.ElementTree as E
...
rows={int(r['official_number_page']):r for r in read(mxl/'reports/mxl-download-manifest.csv') if r['collection']=='1985 English Hymnal'}
missing={int(r['official_number_page']):r for r in read(mxl/'reports/mxl-missing-entries.csv') if r['collection']=='1985 English Hymnal'}
pdfs={int(r['official_number_page']):r for r in read(pdf/'reports/pdf-download-manifest.csv') if r['collection']=='Hymns of The Church of Jesus Christ of Latter-day Saints' and r['variant_type']=='Standard'}
# Full current catalog is exported by Node, avoiding parsing JavaScript as source data.
old=json.loads((root/'test-results/hymnal-before.json').read_text(encoding='utf-8'))
...
by_number={int(s['page']):s for s in old if s['collection']=='Hymns (1985)'};matrix=[]
for number in range(1,342):
 ...
 item.update(id=id,sourceMxl=str(path),sha256=sha,asset=dest,selectedFormat='mxl',state='A',sourceTitle=doc.findtext('./work/work-title') or doc.findtext('./movement-title') or '',keyDeclarations=sorted(set((k.findtext('fifths',''),k.findtext('mode','major')) for k in doc.findall('.//attributes/key'))),lyricElements=len(doc.findall('.//lyric')))
...
(root/'reports/hymnal-1985-inventory.json').write_text(json.dumps({'startingCommit':'a4fdec4','before':list(by_number.values()),'expected':341,'rows':matrix},ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
print('Included',sum(r['state']=='A' for r in matrix),'missing',[(r['number'],r['title']) for r in matrix if r['state']!='A'])
