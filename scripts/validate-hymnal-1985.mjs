...
const {chromium}=createRequire(process.env.PLAYWRIGHT_PACKAGE)('playwright');const b=await chromium.launch({channel:'msedge',headless:true});const p=await b.newPage({viewport:{width:820,height:1180}});const inventory=JSON.parse(fs.readFileSync('reports/hymnal-1985-inventory.json','utf8'));const only=process.env.HYMN_NUMBERS?.split(',').map(Number);const results=only?JSON.parse(fs.readFileSync('reports/hymnal-1985-validation.json','utf8')).filter(r=>!only.includes(r.number)):[];
try{await p.goto(process.env.TEST_URL||'http://127.0.0.1:8768/');await p.locator('.library-row').first().waitFor();
for(const row of inventory.rows.filter(r=>r.asset&&(!only||only.includes(r.number)))){
 ...
 },row);results.push(result);if(results.length%20===0)console.log('Validated',results.length,'/',335,'errors',results.filter(r=>r.error).length);fs.writeFileSync('reports/hymnal-1985-validation.json',JSON.stringify(results,null,2));
}
