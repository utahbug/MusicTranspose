...
## Tempo and expression collision avoidance

OSMD/VexFlow positions metronome groups independently of expression words. Their actual SVG ink bounds can intersect, especially at compact widths. `score-layout.js` checks those bounds after engraving and before screen caching or print cloning. Only colliding metronome groups move upward, retaining their horizontal position and all note/BPM glyphs. A six-SVG-unit gap separates them from text, with repeated checks against other directions, chord labels, fingerings and titles. The SVG canvas expands upward only if moved ink needs clearance. Source XML and musical notation remain unchanged.

The same correction runs for screen and A4 print rendering. Print staging is cleared before each fresh engraving so repeated print preparation cannot accumulate earlier scores; existing page settings and print controls are unchanged. The service worker includes the new local module.

Validation: 48 combinations of four songs, original/+1/-1 keys, and 375x812, 820x1180, 1180x820 and 1440x1000 viewports passed screen/print collision and clipping checks, direction preservation and exact reset. Screen adjustment was needed in all 12 combinations each for Faithful, Silent Night and Shepherd, and 5/12 for Nativity. Print required adjustment for the former three; Nativity's print placement remained unchanged. A synthetic unrelated expression verifies the rule is not title-specific and is idempotent. Opening-system and print screenshots were visually reviewed. Full four-song and minor-key regression suites passed. Physical Safari/iPad testing remains separate from desktop browser viewport emulation.

## Compact opening tempo and expression

The presentation rule reads zero-offset directions before the first timed note in the first part's first measure. A direction containing both metronome and words establishes shared-location evidence. Matching SVG words sit beside the original metronome glyph/BPM group with a comma and normal spacing; all original words remain intact. Available SVG width determines wrapping. The collision check moves the combined group when needed, preserving clearance from other text. Source files and transposition logic are unchanged.
